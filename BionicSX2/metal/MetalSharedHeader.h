...
// AUDIT REFERENCE: Section 3.2, 4.2 — pure C types, no platform dependency
// STATUS: GREEN — zero changes required
...
struct GSMTLConvertPSUniform { int emoda; int emodc; };
...
struct GSMTLInterlacePSUniform { vector_float4 ZrH; };
...
struct GSMTLMainPSUniform
{
    ...
    union { vector_float4 uv_min_max; vector_uint4 uv_msk_fix; };
    vector_float4 lod_params;
    vector_float4 st_range;
    struct { unsigned int blue_mask; unsigned int blue_shift; unsigned int green_mask; unsigned int green_shift; } channel_shuffle;
